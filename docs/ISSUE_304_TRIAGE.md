...
| 구성 조합(마운트 4개 + 사용자 추가 폴더 + `PrinterRedirection=Enable`) | 제보자 **실물 wsb** 와 같은 모양을 제품 직렬화 코드로 재현해 정상 동작(2026-07-28) |
| 시스템 로캘에 따른 wsb 한글 오독 | 아래 0-6-1 참조 |

#### 0-6-1. wsb 에 XML 선언이 없어 한글이 CP949 로 읽힐 가능성 — 기각

`.wsb` 는 XML 선언도 BOM 도 없이 UTF-8 로 기록된다(`File.WriteAllTextAsync` 기본값). 그래서
"파서가 시스템 ANSI 코드 페이지로 읽으면 한글 경로가 깨지지 않는가" 라는 의심이 성립할 수 있다.

그 시나리오는 **"파서가 ANSI 를 쓴다" + "ACP 가 949 다"** 두 조건이 모두 필요한데,
**메인테이너 PC 가 정확히 그 조건**이다(지역 설정의 "Beta: 세계 언어 지원을 위해 Unicode UTF-8 사용"
꺼짐 → `ACP=949`, `OEMCP=949`). 그 상태에서 한글 경로가 든 wsb(BOM·선언 없음)를 샌드박스가
정확히 해석했고, 브레드크럼이 한글 경로 폴더에 그대로 기록됐다. → **파서는 ACP 와 무관하게 UTF-8 을 인식한다.**

반대로 저 베타 옵션이 **켜진**(`ACP=65001`) 환경이라면 ANSI 경로로 읽어도 UTF-8 이 올바르게
디코딩되므로, 우리 파일에 대해서는 효과가 오히려 안전한 쪽이다. 다만 이 옵션은 레거시 구성요소
호환성을 깨는 것으로 알려져 있어, "**ACP=65001 에서 샌드박스의 어떤 구성요소가 오작동한다**" 는
가능성 자체는 배제하지 못한다(메커니즘 미확인, 검증하려면 시스템 로캘 변경 + 재부팅 필요).
제보 환경은 사용자 이름이 `guava`(ASCII)이고 식탁보가 생성하는 경로에는 한글이 들어갈 수 없어
실익이 낮다고 보고 확인하지 않았다.

> 인코딩 위험이 **실재하는 파일은 `.wsb` 가 아니라 `StartupScript.cmd`** 다. 이건 XML 파서가 아니라
> cmd 가 OEM 코드 페이지로 읽으므로, 비 ASCII 가 섞이면 스크립트가 통째로 무동작이 될 수 있다
> (§0-3 하니스 함정). `SandboxStartupScriptTests` 가 순수 ASCII 를 테스트로 고정한다.
...
### 0-6-2. 제보자가 찾은 우회책 — Startup 폴더 마운트 (2026-07-27)

이슈를 닫은 뒤 제보자가 스스로 **동작하는 경로**를 찾아 실물 wsb 와 함께 공유해 주었다
([댓글](https://github.com/yourtablecloth/TableCloth/issues/304#issuecomment-5093285835)).
`<LogonCommand>` 를 **아예 제거**하고, 호스트 폴더를 게스트의 시작 프로그램 폴더에 직접 마운트한 뒤
그 안에 `StartupScript.cmd` 바로 가기를 넣는 방식이다.

```xml
<MappedFolder>
  <HostFolder>C:\Users\<user>\Documents\TableCloth\Startup</HostFolder>
...
- **셸을 통한 `.cmd` 실행은 그 PC 에서 정상**이다(바로 가기 → `.cmd` 가 동작). 파일 연결도 살아 있다.
  → "`.cmd` 연결이 깨졌다" 가설은 기각. 고장난 것은 **LogonCommand 처리 자체**로 좁혀진다.
- 따라서 `cmd.exe /c` 경유 수정(1.20.9)도 그 환경에서는 효과가 없을 가능성이 높다.

#### 제품에 넣을 것인가 — 선택지와 트리거

| 안 | 내용 | 평가 |
| --- | --- | --- |
| **A. 문서화만** | 재설치 안내 + 본 기록 | 비용 0. **다만 사용자 이득은 재설치 안내가 전부** — wsb 는 매 실행 재생성되므로 사용자가 우회책을 직접 적용할 수 없다 |
| **B. Startup 폴백 옵션** | 시작 프로그램 폴더 마운트 + 런처 | `<SandboxFolder>` 배제 원칙을 뒤집어야 하고, 정상 환경에서 **LogonCommand 와 이중 발화** → Global 뮤텍스로 "이미 실행 중" 오류가 나므로 가드 필수. 게다가 증상이 무음이라 **정작 필요한 사용자가 옵션을 찾지 못한다** |
| **C. 호스트 측 실패 감지** | 브레드크럼이 Data 마운트(=호스트 폴더)에 남는 점을 이용해, 일정 시간 안에 `[00]` 이 없으면 호스트가 안내 | `<SandboxFolder>` 불필요, 이중 발화 없음. **무음 실패를 진단된 실패로 바꾼다.** 비용은 B 보다 큼 |

**현재 결정: A 채택, B 보류, C 는 재보고 시.** 확진 사례가 1건이고, 브레드크럼 도입으로 다음 제보는
왕복 한 번에 실패 지점이 확정된다. **동일 증상 2건째가 접수되면 C 를 우선 검토**하고, B 는 C 의
후속(감지 후 폴백 제안)으로 붙이는 것이 자연스럽다.
