...
4. **URL만 온 경우** — 같은 등록 도메인 후보 중 호스트 라벨이 가장 많이 일치하는 **유일** 후보만
   수락한다. 동점이면 URL을 버린다. 한 도메인을 여러 서비스가 공유하는 경우가 실제로 많기 때문이다:
   우리은행 개인(`www.wooribank.com`)/기업(`nbi.wooribank.com`)은 패키지 구성이 다르고(개인만
   Veraport 포함), `fsb.or.kr` 아래에는 저축은행 서비스가 25개 있다. 즉 **생산자가 Id를 같이 넘기는
   것이 정상 경로**이고(익스텐션 팝업이 사용자에게 고르게 한다), URL 단독은 호스트가 카탈로그
   호스트와 충분히 일치할 때만 동작하는 편의 경로다.
