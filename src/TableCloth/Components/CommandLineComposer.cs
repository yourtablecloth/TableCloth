...
public sealed class CommandLineComposer
{
    ...
    public string ComposeCommandLineExpressionForV1(MainWindowViewModel viewModel, bool allowMultipleItems)
    {
        var targetFilePath = this.sharedLocations.ExecutableFilePath;
        var args = this.ComposeCommandLineArgumentsForV1(viewModel, allowMultipleItems);
        return $"\"{targetFilePath}\" {args}";
    }

    public string ComposeCommandLineArgumentsForV1(MainWindowViewModel viewModel, bool allowMultipleItems)
    {
    ...
    public string ComposeCommandLineExpressionForV2(DetailPageViewModel viewModel, bool allowMultipleItems)
    {
        var targetFilePath = this.sharedLocations.ExecutableFilePath;
        var args = this.ComposeCommandLineArgumentsForV2(viewModel, allowMultipleItems);
        return $"\"{targetFilePath}\" {args}";
    }

    public string ComposeCommandLineArgumentsForV2(DetailPageViewModel viewModel, bool allowMultipleItems)
    {
        var options = new List<string>();
